...
const ListPengembalian = () => {
        ...
        if (token) {
          ...
          if (role === "admin") {
            ...
            const studentDetails = await Promise.all(
              borrowingData
                .filter((borrow) => borrow.status === "0") // Filter peminjaman yang belum dikembalikan
                .map(async (borrow) => {
  ...
  const handleStatusChange = async (borrowId) => {
    try {
      // Update status peminjaman di tabel borrowing
      await axios.patch(`http://localhost:3000/borrowing/${borrowId}`, {
        status: "1", // Mengubah status menjadi true (sudah dikembalikan)
      });
